...
public class RegistServlet  extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		String jspName = "RegistViewServlet";				// 遷移先JSP名
		...
		if(ifFrom.equals("regist")){
			...
			Boolean insertCheck = acModel.addProduct(code,name,category,genka,teika,baika,kosu);
			if(!insertCheck){
				System.out.println("商品登録の際にエラーが発生しました");
				s.setAttribute("ERROR", "商品登録エラーが発生しました。");
			}
			insertCheck = null;
			insertCheck = acModel.insertProductKanri(code,groupId);
			if(!insertCheck){
